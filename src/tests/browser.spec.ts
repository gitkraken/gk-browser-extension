...
test('should create a link to open a repository in the repo page', async ({ page }) => {
    ...
    expect(newPage.url()).toBe('https://dev.gk-links.pages.dev/link/dnNjb2RlOi8vZWFtb2Rpby5naXRsZW5zL2xpbmsvci8tP3VybD1odHRwcyUzQSUyRiUyRmdpdGh1Yi5jb20lMkZtaWNyb3NvZnQlMkZ2c2NvZGUuZ2l0');
});
...
test('should create a link to open a commit', async ({ page }) => {
    ...
    const newPage = await pagePromise;
    await newPage.waitForLoadState();
    expect(newPage.url()).toBe('https://dev.gk-links.pages.dev/link/dnNjb2RlOi8vZWFtb2Rpby5naXRsZW5zL2xpbmsvci8tL2MvYmViNTAzYWU4MTMwM2Q2M2FiYjgyMWYyYjBjNjZlNDE2MzNiNDcwNT91cmw9aHR0cHMlM0ElMkYlMkZnaXRodWIuY29tJTJGZ2l0a3Jha2VuJTJGdnNjb2RlLWdpdGxlbnMuZ2l0');
});
