...
<li data-gk class="gl-dropdown-item">
	...
	<div class="gl-dropdown-item-text-wrapper" style="display: flex; align-items: center !important;">
		...
		private handleInsertions(insertions: Map<string, { html: string; position: InsertPosition }>) {
			const localInsertions = insertions;
			for (const [selector, { html, position }] of localInsertions) {
			...
			if (insertions.size) {
				...
				this._observer = new MutationObserver(() => {
					...
					this._timer = setTimeout(() => {
						const timeredInsertions = this.handleInsertions(localInsertions);

						if (!timeredInsertions.size) {
