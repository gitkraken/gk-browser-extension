import type { InjectionProvider, LinkTarget, RenderConfig } from '../provider';
...
export function injectionScope(url: string) {
	class GitHubInjectionProvider implements InjectionProvider {
		...
		private onClick(target: LinkTarget = 'gkdev') {
			const gkUrl = this.tranformUrl(target);
			window.open(gkUrl, '_blank');
		}

		private getRenderConfig(): RenderConfig {
			const [, , , type] = this.uri.pathname.split('/');

			switch (type) {
				case 'commit':
					return {
						selector: '.commit > #browse-at-time-link',
						location: 'after',
						additionalClasses: ['mr-2', 'float-right'],
					};
				case 'tree':
				default:
					return { selector: '.file-navigation get-repo', location: 'before', additionalClasses: ['mr-2'] };
			}
		}

		private render() {
			console.log('rendering');
			const cfg = this.getRenderConfig();
			...
			try {
				...
				button.classList.add('btn', ...(cfg.additionalClasses ?? []));
				button.onclick = () => this.onClick();

				// const details = document.createElement('details');
				// details.classList.add('position-relative', 'details-overlay', 'details-reset', 'mr-2');

				// const summary = document.createElement('summary');
				// summary.style.padding = '1px 8px';
				// summary.classList.add('btn');

				const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
				svg.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
				svg.setAttribute('fill', 'var(--color-btn-text)');
		...
		private tranformUrl(target: LinkTarget): string {
			...
			switch (target) {
				...
				case 'gkdev': {
					...
					gkUrl = new URL(`https://redirect.gitkraken.dev/redirect/${encodeURIComponent(btoa(gkcUrl))}`);
					break;
				}
				case 'vscode':
